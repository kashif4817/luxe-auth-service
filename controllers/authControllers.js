...
export const refreshToken = async (req, res) => {
    try {
        ...
        if (error || !data) return sendResponse(res, 401, "User not found");

        const savedToken = data.refresh_token;
        ...
        jwt.verify(savedToken, process.env.REFRESH_SECRET, (err, decoded) => {
            ...
            sendResponse(res, 200, "new token has been assigned", { newAccessToken });
        });
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (updateError) return sendResponse(res, 404, "User not found or password not updated");
